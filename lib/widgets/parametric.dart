...
class _ParametricPageState extends State<ParametricPage> {
  ...
  Widget _getNotificationState() {
    ...
    if (notifyParametric) return Text('Enabled', style: TextStyle(color: Colors.green),);

    return Text('Disabled', style: TextStyle(color: Colors.red));
  }

  Widget _countdownTimer() {
    if (_endTimeCountdown == -1) return Padding(padding: const EdgeInsets.only(top: 16), child: Util.centerLoadingCircle(''),);

    return CountdownTimer(
      endTime: _endTimeCountdown,
      endWidget: Text('READY!',
        style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red, fontSize: 32),
      ),
  ...
  void _updateOnlineData(String resetTime) async {
    var uid = Util.getFirebaseUid();
    var data = {'parametricReset': DateTime.parse(resetTime).millisecondsSinceEpoch};
    await _db.collection('userdata').doc(uid).set(data, SetOptions(merge: true));
