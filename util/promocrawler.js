...
.then(async text => {
    ...
    for (let i = 0; i < td.length; i+=6) {
        let code = { dateString: td[i].text, reward: td[i+1].text, expired: td[i+2].text, eu: td[i+3].text.removeWhitespace(), na: td[i+4].text.removeWhitespace(), asia: td[i+5].text.removeWhitespace() };
